from empresa import db, login_manager
from empresa import bcrypt
...
class Item(db.Model):
    ...
    def compra(self, usuario):
        ...
        usuario.valor -= self.preco  #decrementar do valor do saldo
        db.session.commit()

    def venda(self, usuario):
        self.dono = None
        usuario.valor += self.preco  #incrementar do valor do saldo
        db.session.commit()